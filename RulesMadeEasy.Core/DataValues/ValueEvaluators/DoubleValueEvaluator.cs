...
{
    /// <summary>
    /// An implementation of an <see cref="IValueEvaluator"/> for <see cref="double"/>s
    /// </summary>
    public class DoubleValueEvaluator : IValueEvaluator
    {
        /// <summary>
        ...
        {
            var valueA = (double)objA;
            var valueB = (double)objB;
            ...
            switch (operation)
            {
                ...
                default:
                    throw new ConditionEvaluationException(ConditionEvaluationException.ExceptionCause.UnsupportedOperator,
                        $"The {nameof(DoubleValueEvaluator)} does not support evaluation for the operator {operation}");
            }
        }
    }
}