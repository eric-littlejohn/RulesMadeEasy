...
{
    /// <summary>
    /// An implementation of an <see cref="IValueEvaluator"/> for <see cref="float"/>s
    /// </summary>
    public class FloatValueEvaluator : IValueEvaluator
    {
        /// <summary>
        ...
        {
            var valueA = (float)objA;
            var valueB = (float)objB;
            ...
            switch (operation)
            {
                ...
                default:
                    throw new ConditionEvaluationException(ConditionEvaluationException.ExceptionCause.UnsupportedOperator,
                        $"The {nameof(FloatValueEvaluator)} does not support evaluation for the operator {operation}");
            }
        }
    }
}