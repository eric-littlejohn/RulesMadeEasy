...
{
    /// <summary>
    /// An implementation of an <see cref="IValueEvaluator"/> for <see cref="byte"/>s
    /// </summary>
    public class ByteValueEvaluator : IValueEvaluator
    {
        /// <summary>
        ...
        {
            byte valueA = (byte)objA;
            byte valueB = (byte)objB;
            ...
            switch (operation)
            {
                ...
                default:
                    throw new ConditionEvaluationException(ConditionEvaluationException.ExceptionCause.UnsupportedOperator,
                        $"The {nameof(ByteValueEvaluator)} does not support evaluation for the operator {operation}");
            }
        }
    }
}