...
{
    /// <summary>
    /// An implementation of an <see cref="IValueEvaluator"/> for <see cref="int"/>s
    /// </summary>
    public class IntValueEvaluator : IValueEvaluator
    {
        /// <summary>
            ...
            switch (operation)
            {
                ...
                default:
                    throw new ConditionEvaluationException(ConditionEvaluationException.ExceptionCause.UnsupportedOperator,
                        $"The {nameof(IntValueEvaluator)} does not support evaluation for the operator {operation}");
            }
        }
    }
}