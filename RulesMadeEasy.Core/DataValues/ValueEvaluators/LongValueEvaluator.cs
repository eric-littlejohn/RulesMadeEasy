...
{
    /// <summary>
    /// An implementation of an <see cref="IValueEvaluator"/> for <see cref="long"/>s
    /// </summary>
    public class LongValueEvaluator : IValueEvaluator
    {
        /// <summary>
            ...
            switch (operation)
            {
                ...
                default:
                    throw new ConditionEvaluationException(ConditionEvaluationException.ExceptionCause.UnsupportedOperator,
                        $"The {nameof(LongValueEvaluator)} does not support evaluation for the operator {operation}");
            }
        }
    }
}